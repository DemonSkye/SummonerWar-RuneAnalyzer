...
import main.java.org.DemonSkye.wut.RuneType.RuneRole;
...
public class RuneDrop {
    public static Double runeEval(String str) {
        ...
        if (grade == 6) {
            ...
            if(slot == 2 || slot == 4 || slot == 6) {
                if (mainStat.contains("DEF +") ||
                        ...
                        ) {System.out.println("Rejected for MainStat base type: " + mainStat);
                        FileOut.writeRune(type, "runeSkip.txt", mainStat, implicit, substat1, substat2, substat3, substat4);
                        return 0.0;
                }
            }
        ...
        if (keep > 10.0) {
            FileOut.writeRune(type, "runeGet.txt", mainStat, implicit, substat1, substat2, substat3, substat4);
            return 2.0;
        }
        if(keep <10.0 && keep >=7.0) {
            FileOut.writeRune(type, "runeMaybe.txt", mainStat, implicit, substat1, substat2, substat3, substat4);
            return 1.0;
        }
        else{
            FileOut.writeRune(type, "runeSkip.txt", mainStat, implicit, substat1, substat2, substat3, substat4);
